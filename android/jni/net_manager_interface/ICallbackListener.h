...
namespace android {
	...
	public:
		...
		//
		virtual int SetReadCallback	(fnMgmtIoCallback readcallback) = 0;
		virtual int SetWriteCallback(fnMgmtIoCallback writecallback) = 0;
		virtual int SetNotifyCallback(fnMgmtNotifyCallback notifycallback) = 0;
		virtual int SetLogExpCallback(fnMgmtLogExportCallback logexpcallback) = 0;
		virtual int SetGetValueCallback(get_value_func getcallback) = 0;
		virtual int SetSetValueCallback(set_value_func setcallback) = 0;
		virtual int SetCapabilityCallback(get_capability_func capabilitycallback) = 0;
		virtual char* GetValue(char *name) = 0;
		virtual void SetValue(char *name, char *str, unsigned int pval) = 0;
		virtual String16 GetCapability() = 0;
	};
