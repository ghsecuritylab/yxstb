    ...
    IN STUN_BYTE octet)
{
    ...
    if ( messageDescriptor->offset >= messageDescriptor->bufferSize ) {
        TR069Printf( "Error: Message's offset is lager then bufferSize\n");
        return ;
    }
    ...
    IN STUN_DWORD valueSize)
{
    ...
    if( value == NULL && valueSize > 0 ) {
        TR069Printf( "Error: !( value == NULL && valueSize > 0 )\n" );
        return;
    }
    ...
    OUT STUN_BYTE *octet)
{
    ...
    if ( messageDescriptor->offset >= messageDescriptor->bufferSize ) {
        TR069Printf("The buffer is too small\n");
        return -1;
    }
    ...
    IN STUN_DWORD variableSize)
{
    ...
    if ( messageDescriptor->buffer == NULL ) {
        TR069ErrorOut("Allocate messsage's buffer failed\n" );
    }
    ...
    IN STUN_DWORD messageSize)
{
    ...
    if(messageSize < 2){
        TR069Printf("Message is too short! \n");
        return RESULT_ERROR;
    }
    ...
    OUT STUN_MESSAGE_DESCRIPTOR *messageDescriptor )
{
    ...
    switch(attrType){
        ...
        case MESSAGE_INTEGRITY:
        {
            ...
            tr069_api_getValue("Device.ManagementServer.STUNPassword", password, STUN_STRING_MAX_LEN);
            ...
            //TR069Printf("stunPassword: %s, len = %d\n", password, strlen(password));ÃÜÂë²»ÄÜ´òÓ¡
    ...
    OUT STUN_MESSAGE *parsedMsg)
{
    ...
    while(messageDescriptor->offset < messageDescriptor->bufferSize){
        ...
        TR069Printf("Message attribute type : %u, size = %u\n", msgAttrType, msgAttrSize);
        ...
        switch(msgAttrType){
            ...
            case BINDING_CHANGE:
                ...
                break;
            default:
                TR069Printf("Unrecognized stun message attribute.\n");
                break;
        }
    }
    ...
    OUT STUN_DWORD *pMessageSize)
{
    ...
    if( NULL == pMessage || pMessageSize == NULL ){
        TR069ErrorOut("pMessage==NULL || pMessageSize == NULL\n");
    }
    
    TR069Printf("Binding request type= %d\n", type);
    switch(type){
        ...
        case BINDING_CHANGE_AUTHORIZED:
            ...
            break;
        default:
            TR069ErrorOut("Not support this binding request, type= %d\n", type);
            break;
    }
    ...
    if( needAuth ){
        tr069_api_getValue("Device.ManagementServer.STUNUsername", userName, STUN_STRING_MAX_LEN);
    ...
    *pMessageSize = messageDescriptor->bufferSize;
    ...
    TR069Printf("userName = %s, bufferSize = %d, offset =%d\n", userName,
    ...
    OUT STUN_MESSAGE_DESCRIPTOR **messageDescriptor )
{
    ...
    if(stun_UnpackWord(localMsgDescriptor, &messageType)){
        TR069ErrorOut("unpack message type error\n");
    }

    TR069Printf("received message type: %#x\n", messageType);
    
    if((messageType != BINDING_RESPONSE) && (messageType != BINDING_ERROR_RESPONSE)){
        TR069ErrorOut("Invalid. is not binding response message\n");
    }
    ...
    OUT TR069_STUN_MESSAGE *validInfo )
{
    ...
    if(RESULT_SUCCESS != retVal){
        TR069ErrorOut(" unpack binding reponse message error\n");
    }

    /* Verify transaction ID */
    if(memcmp(validInfo->bindingReqID.val128, parsedMsg.header.id.val128, 16)){
        TR069ErrorOut("invalid binding response\n");
    }
    ...
    if(parsedMsg.hasMessageIntegrity){
        if( validInfo->type == BINDING_REQ_BASIC ||
            ...
            TR069ErrorOut("request message is basic, but response need integrity\n");
        }else{
            validInfo->type = BINDING_REQ_BASIC;
        }

        tr069_api_getValue("Device.ManagementServer.STUNPassword", stunpssword, STUN_STRING_MAX_LEN);
        ...
        if(memcmp(parsedMsg.messageIntegrity.value, digest, STUN_SHA1_DIGEST_LEN)){
            TR069ErrorOut("Reponse message is not integrity. \n");
        }
    }
    ...
    if(parsedMsg.hasErrorCode){
        ...
        TR069Printf("errorCode = %d\n", errorCode);

        if(errorCode == 401){
            TR069Printf("Unautheorized\n");
            if( validInfo->type == BINDING_REQ_BASIC){
    ...
    if(parsedMsg.hasMappedAddr) {
        ...
        if(validInfo->localAddr.address != parsedMsg.mappedAddr.address){
            if(1 != validInfo->natDetected){
                ...
                tr069_api_setValue("Device.ManagementServer.UDPConnectionRequestAddress", notifyAddr, 0);
                tr069_api_setValue("Device.ManagementServer.NATDetected", "1", 0);
            ...
            if(0 != validInfo->natDetected){
                tr069_ipAddrInt2Dot(parsedMsg.mappedAddr.address, addrDotFormat, sizeof(addrDotFormat));
                sprintf(notifyAddr, "%s:%u", addrDotFormat, parsedMsg.mappedAddr.port);
                tr069_api_setValue("Device.ManagementServer.UDPConnectionRequestAddress", notifyAddr, 0);
                tr069_api_setValue("Device.ManagementServer.NATDetected", "0", 0);
    ...
    OUT TR111_UDP_CONNECT_REQUEST *connRequest )
{
    ...
    if(item != 3 || strcasecmp(req->method, "GET") || strcasecmp(req->version,"HTTP/1.1")){
        TR069ErrorOut("http request line is invalid\n");
    }
    ...
    if(item != 5){
        TR069ErrorOut("Querey string parse error\n");
    }

    TR069Printf("method:%s,uri:%s,version:%s,host:%s, port=%u"
    ...
    OUT TR069_STUN_MESSAGE *validInfo )
{
    ...
    if(RESULT_SUCCESS != stun_UnpackUPDConnectRequestMsg( message, messageSize, &connRequest )){
        TR069ErrorOut(" unpack UDP connect request message error\n");
    }
	
    if( validInfo->conReqID == connRequest.id){
        TR069ErrorOut(" Message ID is same as last valid ID\n");
    }

    tr069_api_getValue("Device.ManagementServer.ConnectionRequestUsername", conReqUserName, STUN_STRING_MAX_LEN);
    if(strcmp(conReqUserName, connRequest.un )){
        TR069ErrorOut(" Connection userName is invalid\n");
    }

    tr069_api_getValue("Device.ManagementServer.ConnectionRequestPassword", conReqPassword, STUN_STRING_MAX_LEN);
    ...
    TR069Printf("Text:%s, digestHexa = %s,connRequest.sig = %s\n",  text, digestHexa, connRequest.sig);
...
#else
    {
        static STUN_BYTE *gMessage = NULL;
        static STUN_DWORD gMessageSize = 0;

        if (gMessage && gMessageSize == messageSize) {
            if (0 == memcmp(gMessage, message, messageSize))
                return RESULT_SUCCESS;
        }
        if (gMessage)
            free (gMessage);
        gMessage = (STUN_BYTE *)malloc(messageSize);
        if (gMessage) {
            memcpy(gMessage, message, messageSize);
            gMessageSize = messageSize;
        }
    }
    ...
    IN STUN_DWORD sendMsgSize)
{
    ...
    int i;
	  ...
	  tv.tv_sec = 5;
    tr069_api_getValue("Device.ManagementServer.STUNServerAddress", stunServerAddr, 16);
    {
        char buf[12];
        buf[0] = 0;
        tr069_api_getValue("Device.ManagementServer.STUNServerPort", buf, 12);
        stunServerPort = atoi(buf);
    }
    ...
    servaddr.sin_port = htons(stunServerPort);
	  ...
	  for(i = 0;i < 3; i++){ 
	    ret = tr069_sendto(sockfd, sendMsg, sendMsgSize, 0, &servaddr); 
    if (ret != -1) 
	      break; 
	  select(1, NULL, NULL, NULL, &tv); 
	  } 
	
    TR069Printf("sendto ret = %d\n",ret); 
return ret; 
}
    ...
    IN STUN_INT arg)
{
    ...
    TR069Printf("send STUN message ID = %u\n", id);
    msg.id = id;
    msg.arg0 = arg;
