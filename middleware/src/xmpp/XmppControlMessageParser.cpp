...
XmppControlMessageParser::XmppControlMessageParser(XmppService *xmppService)
    : m_xmppService(xmppService)
{
}
XmppControlMessageParser::~XmppControlMessageParser()
{
}

void
XmppControlMessageParser::startPlayEncapsulate(struct json_object *obj)
{
...
void
XmppControlMessageParser::trickPlayControlEncapsulate(struct json_object *obj)
{
    int seekPosition = 0;
    int fastSpeed = 0;
...
void
XmppControlMessageParser::channelListEncapsulate()
{
   //implement it later ...
   return;
}
...
void
XmppControlMessageParser::pictureShowEncapsulate(struct json_object *obj)
{
...
void
XmppControlMessageParser::remoteControlHandler(struct json_object *obj)
{
...
void
XmppControlMessageParser::setVolumeHandler(struct json_object *obj)
{
    return;
}

void
XmppControlMessageParser::playerStateEncapsulate()
{
...
void
XmppControlMessageParser::playerVolumeEncapsulate()
{
    int volume = 0;
    char jsonBuffer[1024] = {0};
...
void
XmppControlMessageParser::parseActionOfFunctionCall(struct json_object *obj)
{
...
void
XmppControlMessageParser::parseActionOfEventInfo(struct json_object *obj)
{
...
void
XmppControlMessageParser::parseActionOfGetAppState(struct json_object *obj)
{
...
#if 0 // Just for test.
void
XmppControlMessageParser::parseActionOftest(struct json_object *obj) //just for test.....
{
...
void
XmppControlMessageParser::startParseMessage(const Message& message)
{
    xmppLogDebug("%s\n", message.body().c_str());

    if (!message.body().size() || !strchr(message.body().c_str(), '{')) { //这里有个死机问题需要查清楚，当为数字开头的时候
        xmppLogError("Invalid data.\n");
        return;
    }

    if (gloox::Message::Sysctrl == message.subtype()) {
        struct json_object *obj = json_tokener_parse_string(message.body().c_str());
        if (!obj) {
