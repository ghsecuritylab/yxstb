...
void mad_header_init(struct mad_header *header)
{
  ...
  header->duration       = mad_timer_zero1;
}
...
PRINTF("MAD_ERROR_BADLAYER\n");
  ...
  /* bitrate_index */
  tIndex = mad_bit_read(&stream->ptr, 4);

  if (tIndex == 15) {
PRINTF("MAD_ERROR_BADBITRATE\n");
    stream->error = MAD_ERROR_BADBITRATE;
    return -1;
  }

  if (header->flags & MAD_FLAG_LSF_EXT)
    header->bitrate = bitrate_table[3 + (header->layer >> 1)][tIndex];
  else
    header->bitrate = bitrate_table[header->layer - 1][tIndex];

  /* sampling_frequency */
  tIndex = mad_bit_read(&stream->ptr, 2);

  if (tIndex == 3) {
PRINTF("MAD_ERROR_BADSAMPLERATE\n");
    stream->error = MAD_ERROR_BADSAMPLERATE;
    return -1;
  }

  header->samplerate = samplerate_table[tIndex];
