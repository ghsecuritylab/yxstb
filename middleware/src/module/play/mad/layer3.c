				  ...
				  struct channel *gr1ch, int mode_extension)
{
  ...
  unsigned int scalefac_compress, tIndex, slen[4], part, n, i;
  unsigned char const *nsfb;

  start = *ptr;

  scalefac_compress = channel->scalefac_compress;
  tIndex = (channel->block_type == 2) ?
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    ...
    PRINTF("@@@@@@@@: malloc: %p / %d\n", stream->main_data, MAD_BUFFER_MDLEN);
    if (stream->main_data == 0) {
PRINTF("MAD_ERROR_NOMEM\n");
  ...
  if (frame->overlap == 0) {
    ...
    PRINTF("@@@@@@@@: malloc: %p / %d\n", frame->overlap, 2 * 32 * 18 * sizeof(mad_fixed_t));
    if (frame->overlap == 0) {
PRINTF("MAD_ERROR_NOMEM\n");
      stream->error = MAD_ERROR_NOMEM;
      return -1;
...
PRINTF("error = %d\n", error);
  ...
  {
    struct mad_bitptr peek;
    unsigned long tHeader;

    mad_bit_init(&peek, stream->next_frame);

    tHeader = mad_bit_read(&peek, 32);
    if ((tHeader & 0xffe60000L) /* syncword | layer */ == 0xffe20000L) {
      if (!(tHeader & 0x00010000L))  /* protection_bit */
	mad_bit_skip(&peek, 16);  /* crc_check */

      next_md_begin =
	mad_bit_read(&peek, (tHeader & 0x00080000L) /* ID */ ? 9 : 8);
    }
