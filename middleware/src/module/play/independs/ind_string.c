...
int ind_lineparse(char* string, char* name, int namelen, char* value, int valuelen)
{
	...
	if (string == NULL || name == NULL || namelen <= 0 || value == NULL || valuelen <= 0)
		ERR_OUT("string = %p, name = %p, namelen = %d, value = %p, valuelen = %d\n", string, name, namelen, value, valuelen);
	...
	l = i - j;
	if (l <= 0)
		ERR_OUT("name is NULL");
	if (l >= namelen)
		ERR_OUT("l = %d, namelen = %d\n", l, namelen);
	IND_MEMCPY(name, string + j, l);
	name[l] = 0;
	...
	l = i - j;
	if (l >= valuelen)
		ERR_OUT("l = %d, valuelen = %d\n", l, valuelen);
	IND_MEMCPY(value, string + j, l);
	value[l] = 0;
...
int ind_strline(char *string, char *buf, int len)
{
	...
	if (l > 0)
		IND_MEMCPY(buf, string, l);
	buf[l] = '\0';

	return l;
}

void ind_strdup(char **pp, char *str)
{
    char *p = *pp;
    if (p)
        IND_FREE(p);
