...
static	void			jpgfreeall(Header *h, int freeimage);
...
Rawimage*
img_decode_jpg(uint8_t *imgbuf, int imglen)
{
	Rawimage* image;
	Biobuf b;
	...
	while(header->err[0] == '\0'){
		...
		case APPn+0:
			if (nseg==1 && strncmp((char*)b, "JFIF", 4)==0)  /* JFIF header; check version */
				if (b[5]>1 || b[6]>2)
					ERR_OUT("ReadJPG: can't handle JFIF version %d.%2d", b[5], b[6]);
			break;
		...
		case  SOS:
			...
			case SOF2:
				...
				break;
			default:
				ERR_OUT("unrecognized or unspecified encoding %d", header->mode);
				break;
			}
			break;
		...
		default:
			ERR_OUT("ReadJPG: unknown marker %.2x", m);
			break;
		}
	}
Err:
...
marker(Header *h)
{
	...
	while((c=readbyte(h)) == 0)
		fprint(2, "ReadJPG: skipping zero byte at offset %d\n", Boffset(h->fd));
	if (c != 0xFF)
		jpgerror(h, "ReadJPG: expecting marker; found 0x%x at offset %d\n", c, Boffset(h->fd));
	while(c == 0xFF)
		c = readbyte(h);
	return c;
}
...
readsegment(Header *h, int *markerp)
{
	...
	case EOI:
		*markerp = m;
		return 0;
	case 0:
		jpgerror(h, "ReadJPG: expecting marker; saw %.2x at offset %d", m, Boffset(h->fd));
	}
	...
	for (mcu=0; mcu<nmcu; ){
		for (comp=0; comp<Ns; comp++){
			...
			for (block=0; block<H[comp]*V[comp]; block++){
				...
				/* F-23 */
				zz = data[comp][block];
				IND_MEMSET(zz, 0, 8*8*sizeof(int));
				zz[0] = qt[0]*DC[comp];
				k = 1;
				...
				for (;;){
					...
					if ((t&0x0F) == 0){
						...
						k += t>>4;
                		if (k < 0 || k >= 64)
                			jpgerror(h, "bad Huffman code k = %d\n", k);
						z = receive(h, t&0xF);
						zz[zig[k]] = z*qt[k];
						if (k == 63)
							break;
...
restart(Header *h, int mcu)
{
	...
	if (nskip != 2)
		jpgerror(h, "ReadJPG: skipped %d bytes at restart %d\n", nskip-2, rest);
	if (rst < 0)
		jpgerror(h, readerr);
	if ((rst&7) != (rest&7))
...
progressiveIDCT(Header *h, int colorspace)
{
	...
	IND_MEMSET(blockno, 0, sizeof blockno);
...
colormap(Header *h, int colorspace, Rawimage *image, int *data0[8*8], int *data1[8*8], int *data2[8*8], int mcu, int nacross, int Hmax, int Vmax,  int *H, int *V)
{
	...
	int t, b0, b1, b2, tY0, tY1, y2, x0, x1, x2;
	int Y, Cr, Cb;
	...
	for (y=0; y<dy; y++){
		...
		for (x=0; x<dx; x++){
			if (colorspace == CYCbCr){
				rpic[pici+x] = clamp[data0[b0][tY0+x0++*H0/Hmax] + 128 + CLAMPOFF];
				gpic[pici+x] = clamp[data1[b1][tY1+x1++*H1/Hmax] + 128 + CLAMPOFF];
				bpic[pici+x] = clamp[data2[b2][y2+x2++*H2/Hmax] + 128 + CLAMPOFF];
			}else{
				Y = (data0[b0][tY0+x0++*H0/Hmax]+128)<<11;
				Cb = data1[b1][tY1+x1++*H1/Hmax];
...
decode(Header *h, Huffman *t)
{
	...
	if (i >= 17){
		if (badcode == 0)
			jpgerror(h, "badly encoded %dx%d JPEG file\n", h->X, h->Y);//2012-12-3 21:07:32 ljh fprint ÐÞ¸ÄÎª jpgerror
		badcode = 1;
		i = 0;
	}
	h->cnt = cnt;
...
int img_info_jpg(uint8_t *imgbuf, int imglen, int *pwidth, int *pheight)
{
	...
	while(1) {
		...
		if (buf[0] != 0xff)
			ERR_OUT("sync\n");
		while (buf[1] == 0xff) {
		if (Bread(&b, buf + 1, 1) != 1)
			ERR_OUT("Bread 1\n");
		}
		//PRINTF("Bread offset = 0x%04x, segment = %02x\n", Boffset(&b) - 2, buf[1]);
		...
		case SOF2:
			...
			if (Bread(&b, buf, 2) != 2)
				ERR_OUT("Bread\n");
			*pheight = (int)(((uint32_t)buf[0] << 8) + (uint32_t)buf[1]);
			if (Bread(&b, buf, 2) != 2)
				ERR_OUT("Bread\n");
			*pwidth = (int)(((uint32_t)buf[0] << 8) + (uint32_t)buf[1]);
			return 0;
		default:
			if (Bread(&b, buf, 2) != 2)
				ERR_OUT("Bread\n");
			l = (int)(((uint32_t)buf[0] << 8) + (uint32_t)buf[1]) - 2;
			if (Bread(&b, NULL, l) != l)
				ERR_OUT("Bread offset = 0x%04x, l = %d\n", Boffset(&b), l);
		}
	}
