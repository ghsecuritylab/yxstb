...
public:
    IptvLogMessage(const char * file, const char * func, int line, bool fatal = false);
    IptvLogMessage();
    ~IptvLogMessage();
...
public:
    ScopeTracer(const char * func, const char * name, int line) : name_(name), func_(func), line_(line) {
        IptvLogMessage().stream() << TERMC_RED << "Into scope: " << name << " (" << func << ':' << line << ")" << TERMC_NONE;
    ...
    ~ScopeTracer() {
        ...
        IptvLogMessage().stream() << TERMC_GREEN << "Leave scope: " << name_ << " (" << func_ << ':' << line_ << ")" << TERMC_BLUE << " Time costs: " << tu << " us." << TERMC_NONE;
    }
private:
    const char * name_;
    const char * func_;
...
public:
    ...
    ~TimerScopeTracer() {
        ...
        long long tu = ((long long)tv.tv_sec * 1000000 + tv.tv_usec) - ((long long)start_.tv_sec * 1000000 + start_.tv_usec);
        if (tu >= difftime_) {
            IptvLogMessage().stream() << TERMC_PINK << "Leave scope: " << name_ << " (" << func_ << ':' << line_ << ")" << TERMC_BLUE << " Time costs: " << tu << " us." << TERMC_NONE;
        }
    }
private:
    const char * name_;
...
#define ILOG()   ::logging::LogMessageVoidify() & ::logging::IptvLogMessage(__FILE__, __FUNC__, __LINE__).stream()
#define FATAL()  ::logging::LogMessageVoidify() & ::logging::IptvLogMessage(__FILE__, __FUNC__, __LINE__, true).stream() << TERMC_RED << "FATAL: "
