...
extern "C"{
extern void LocalPlayerDiskRefresh(int type, const char *diskName, int position, const char *mountPoint);
...
extern void mid_net_mac_addr(char *addr, char pDiv);
#ifdef INCLUDE_cPVR
...
int DiskManager::refresh()
{
            ...
            if ( m_disk.find(m_removeDiskName.at(i)) != m_disk.end()) {
#ifdef INCLUDE_LocalPlayer
                {
                    ...
                    if (count > 0) {
                    	...
                    	LocalPlayerDiskRefresh(0, disk->getName().c_str(), 0, mountPoint.c_str());
                    }
                    delete disk;
                }
#endif
                ...
                DiskEvent diskEvent;
                diskEvent.build("HDD_REMOVED", m_removeDiskName.at(i).c_str(), pvrFlag);
                diskEvent.report();
    ...
    if (m_addDiskName.size() > 0) {//new disk attached
       for(int i = 0; i < m_addDiskName.size(); i++) {
            ...
            DiskEvent diskEvent;
            diskEvent.build("HDD_ATTATCHED", m_addDiskName.at(i).c_str(), pvrFlag);
            diskEvent.report();
            diskEvent.build("HDD_PARTITION_MOUNTED", m_addDiskName.at(i).c_str(), pvrFlag);
            diskEvent.report();
       }
       m_addDiskName.clear();
    }
...
int DiskManager::pvr_md5_get(char dest[32 + 1])
{
	unsigned char buf[22 + 1] = {0};//macµØÖ·ºÍ99090
	int i;
	unsigned char c;
	MD5_CTX ctx;
	...
	memset(dest, 0, 32);
	MD5_Init(&ctx);
    network_tokenmac_get((char*)buf, 22, '-');
	strcat((char*)buf, "99090");
	MD5_Update(&ctx, buf, 22);
	MD5_Final(buf, &ctx);
	for(i = 0; i < 16; i ++) {
		c = buf[i] >> 4;
		if (c < 10)
