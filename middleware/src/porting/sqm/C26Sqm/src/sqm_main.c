...
Àı×Ó:/home/hybroad/bin/sqmloader -c  /var/sqm.ini  -s  /root
*/
static void startSqmpro(void)
{
    SQM_PRINT("sqm_main::Child process PID = %d\n", getpid());
    system("/home/hybroad/bin/sqmloader -c  /var/sqm.ini  -s  /root");
    SQM_PRINT("sqm_main: /home/hybroad/bin/sqmloader -c  /var/sqm.ini  -s  /root\n");
		...
		if(ret > 0){
			if(FD_ISSET(fd_mid_to_sqm, &fdSets)){
				...
				case SQM_CMD_STOP_MONITOR:
					...
					SQM_PRINT("killall -9 sqmloader\n");
					SQM_PRINT("killall -9 sqmpro\n");
					break;
				default:
