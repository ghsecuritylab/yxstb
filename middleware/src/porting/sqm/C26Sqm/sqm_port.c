...
int sqm_port_pushmsg(int pIndex, SQM_MSG_C26 *sqm_msg_c26, int len, SQM_MSG sqm_msg)
{
...
int sqm_file_check(void)
{
    ...
    if((-1 != find) && (!find)) {
        if(!sqmpro_flag)
            pthread_create(&pHandle_sqm, NULL, (void *)sqmpro_copy, 0);
        return 1;
    } else
        return 0;
}
...
int sqm_get_epg_ip(char  *return_ip, int ip_len)
{
    ...
    app_epgUrl_get(epg_temp);	// get epg url, exp: http://10.10.10.39:33200/EPG/jsp
    if(strlen(epg_temp)) {
...
int sqm_info_copy(char *writePath)
{
    ...
    //获取机顶盒网卡
    if(ret == 1)
        IND_STRCPY(sqm_file.StbNetName, "ppp0"); //TODO TODO TODO
    else
