...
char *HwSoftwareVersion(int type)
{
    char version[64] = {0};
if (1 == type)
    strncpy(version, "V100R002C20LBAS01B321", sizeof(version));
else
    strncpy(version, "IPTV STB V100R002C20LBAS01B321", sizeof(version)); //huawei V5 base
    static char huaweiVer[256] = {0};
#if (defined DEBUG_BUILD)
#if defined(BROWSER_INDEPENDENCE)
    snprintf(huaweiVer, 256, "%s[D-NEW]", version);
#else //BROWSER_INDEPENDENCE
    snprintf(huaweiVer, 256, "%s[D-OLD]", version);
